...
The reading of "is" for value equality is **context-sensitive**:

- In **expression / condition position**, `x is [literal]` is value equality. A literal
  right-hand side (a number, string, or boolean) is neither a type nor a predicate, so
  there is no ambiguity — and `if x is 5` is the natural English reading.
- In **statement position**, bare `x is [literal].` is **forbidden** — there `is`
  collides with assignment ("set x to 5" vs. "assert x == 5"). Use `Let`/`Set` to assign
  or `equals` to compare.

**The Uses of "Is":**

| Syntax | Position | Meaning | Compiles To |
|--------|----------|---------|-------------|
| `If x is 5:` | condition / expression | Value equality (literal RHS is unambiguous) | `x == 5` |
| `x equals 5` | any | Value equality | `x == 5` |
| `x is a User` | any | Type check | `matches!(x, User { .. })` |
| `x is valid` | any | Predicate application | `x.is_valid()` |

**Forbidden (statement position only):**

| Pattern | Error |
|---------|-------|
| `x is 5.` (bare statement) | Ambiguous with assignment — use `Let x be 5` to assign or `x equals 5` to compare |

**Rationale:**

- `is a [Type]` → type check (article + PascalCase = unambiguous)
- `is [adjective]` → predicate (lowercase = unambiguous)
- `is [literal]` in a **condition** → equality (a literal RHS is neither a type nor a
  predicate, so it is unambiguous)
- `is [literal]` as a **statement** → forbidden (ambiguous with assignment)

**Error Message (statement position):**

```
Error at line 12: Use 'equals' for value comparison (or 'Let'/'Set' to assign).

  12 │ count is 5.
     │          ^^
     │
  = In a statement, 'is' is ambiguous with assignment.
  = To compare: If count equals 5.   To assign: Let count be 5.
```
