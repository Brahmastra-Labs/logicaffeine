//! The LOGOS Build Tool (CLI) — project pipeline integration tests.
//!
//! Tests for project creation, building, and execution.
//!
//! Moved from `crates/logicaffeine_tests/tests/phase37_cli.rs`, where a
//! never-wired `cli` feature gate kept them permanently compiled out. Living
//! in the CLI crate itself, they run unconditionally with the workspace suite.

use std::fs;
use tempfile::tempdir;

use logicaffeine_cli::project::build::{build, find_project_root, BuildConfig};
use logicaffeine_cli::project::manifest::Manifest;
