//! Privacy policy page.
//!
//! Renders the privacy policy from an embedded HTML file with consistent
//! legal page styling and navigation.
//!
//! # Route
//!
//! Accessed via `Route::Privacy`.
...
// so the ~110 KB of legal prose never rides inside the shipped binary.
...
pub fn Privacy() -> Element {
    let breadcrumbs = vec![
        ...
        BreadcrumbItem { name: "Privacy Policy", path: "/privacy" },
    ];
    let schemas = vec![
        organization_schema(),
        webpage_schema("Privacy Policy", "How LOGICAFFEINE collects, uses, and protects your personal information.", "/privacy"),
    ...
    rsx! {
        PageHead {
            ...
            canonical_path: seo_pages::PRIVACY.canonical_path,
        }
        style { "{LEGAL_STYLE}" }
        JsonLdMultiple { schemas }

        div { class: "legal-container",
            MainNav { active: ActivePage::Other, subtitle: Some("Privacy Policy") }
...
#[cfg(not(target_arch = "wasm32"))]
#[component]
fn PrivacyBody() -> Element {
    rsx! {
        div { class: "legal-content-inner", dangerous_inner_html: "{PRIVACY_HTML}" }
    }
}
...
#[component]
fn PrivacyBody() -> Element {
    let mut body = use_resource(|| crate::ui::data_fetch::fetch_static_text("/data/privacy.html"));
    ...
    match &*state {
        ...
        Some(Err(e)) => rsx! {
            div { class: "legal-content-inner",
                p { "The privacy policy failed to load: {e}" }
