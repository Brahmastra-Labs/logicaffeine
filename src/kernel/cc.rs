...
//!
//! Proves equalities over uninterpreted functions using
//! Union-Find with congruence propagation.
//!
//! The cc tactic handles:
//! - Direct equalities: (Eq a a) by reflexivity
//! - Implications with hypotheses: (implies (Eq x y) (Eq (f x) (f y)))
//!
//! Algorithm: Downey-Sethi-Tarjan (simplified)
//! 1. Build E-graph from all subterms
//! 2. Add hypothesis equalities as merges
//! 3. Propagate congruences until fixed point
//! 4. Check if goal's LHS and RHS are in the same equivalence class
...
/// E-graph node: represents a term in the E-graph
...
pub enum ENode {
    ...
    Name(String),
    /// Application (curried): func applied to arg
...
pub struct EGraph {
    nodes: Vec<ENode>,
    uf: UnionFind,
    /// Map from node content to term ID (hash-consing)
    node_map: HashMap<ENode, TermId>,
    /// Pending merges for congruence propagation
    pending: Vec<(TermId, TermId)>,
    /// Use lists: for each term, parent apps that use it
...
/// Reify a Syntax term into an EGraph, returning the root TermId
...
/// Returns (hypotheses, conclusion) where hypotheses are equality pairs.
