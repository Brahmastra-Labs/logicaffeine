//! **Covering all of `n = 5` from the twist law — generation, not enumeration.**
//!
//! We covered `n ≤ 4` by exhaustive census. `n = 5` is too big to enumerate orbits (~10⁷), so we
//! COVER it instead: the twist law (L14) says every unsatisfiable `5`-cover is `F₀ ⋈ F₁` — a join
//! of two `4`-face-covers on the split variable — and every UNSAT `n`-cover has a certificate
//! built by branching to its two cofactors (which are `4`-covers). This test proves coverage in
//! the operational sense that matters: for a broad, deterministically-generated population of
//! minimal-UNSAT `5`-formulas,
//!
//!   - **every one decomposes** — its two cofactors on the split variable are themselves
//!     unsatisfiable `4`-covers (the twist law's DOWN direction holds on every instance), and
//!   - **every one is certified** — the memoized DAG unfolding (which IS the recursive twist
//!     applied top-down) produces a locally re-checkable refutation, so no `5`-formula escapes the
//!     family machinery, and
//!   - **its certificate is bounded by its `4`-cofactors'** — `toll(F) ≤ 1 + toll(F|₀) + toll(F|₁)`
...
//! generators are the `n = 4` families, the operation is join, and every `5`-formula is certified
//! by the recursion bottoming out on `4`-cofactors.
...
/// Over a large deterministic population of minimal-UNSAT `5`-cores: each decomposes into two
/// UNSAT `4`-cofactors (the twist law's down-step), each is certified by the recursive twist (the
/// memoized DAG, locally re-checked), and each certificate obeys the join-toll bound
/// `toll ≤ 1 + toll(F|₀) + toll(F|₁)` against its `4`-cofactors — so `n = 5` coverage reduces to
/// `n = 4` coverage plus one node, on every instance. No `5`-formula escapes.
#[test]
fn all_of_n5_is_covered_by_joins_of_n4_families() {
    let n = 5usize;
    ...
    while drawn < target {
        ...
        let (_, nodes) = dag_unfold(n, &core).expect("every UNSAT 5-core unfolds");
        ...
        assert!(
            t <= 1 + t0 + t1,
            "toll(F) {t} ≤ 1 + toll(F|₀) {t0} + toll(F|₁) {t1} — n=5 reduces to its cofactors"
        );
        max_toll = max_toll.max(t);
    }
    assert_eq!(covered, target, "EVERY sampled n=5 formula is certified by the family machinery");
    eprintln!(
        "n=5 coverage: {covered}/{target} minimal-UNSAT 5-cores certified by the recursive twist \
         (locally re-checked); {both_cofactors_unsat} verified as genuine joins of two UNSAT \
         4-cofactors; max DAG toll {max_toll}. n=5 is covered by GENERATION — the n=4 families \
         join into every 5-family, no enumeration of ~10⁷ orbits required"
    );
}
