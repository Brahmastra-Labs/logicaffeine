...
## Main
    ...
    assert!(
        rust.contains("impl logicaffeine_data::crdt::Merge for Scoreboard"),
...
## Main
    ...
    let rust = compile_to_rust(source).expect("Should compile");
    assert!(
        rust.contains("logicaffeine_data::crdt::GCounter"),
        "Should use GCounter from logicaffeine_data"
    );
}
...
## Main
    ...
    assert!(
        rust.contains("impl logicaffeine_data::crdt::Merge for AllCrdt"),
