...
fn test_mvregister_empty_then_set() {
    use logicaffeine_data::crdt::MVRegister;
...
fn test_orset_add_wins_vs_remove_wins_same_scenario() {
    use logicaffeine_data::crdt::{AddWins, Merge, ORSet, RemoveWins};
...
fn test_lww_exact_timestamp_tiebreak() {
    ...
    // Looking at lww.rs:73 - "If timestamps are equal, the other value wins"

    // Create two registers with the same timestamp but different values
    let mut a = LWWRegister::new("a".to_string(), 1000);
    let b = LWWRegister::new("b".to_string(), 1000);

    // When we merge b into a with same timestamp, b's value should win
    // (per the >= comparison in merge)
    a.merge(&b);
    assert_eq!(a.get(), "b"); // Other value wins on tie
}
...
fn test_lww_newer_always_wins() {
    ...
    let mut r1 = LWWRegister::new("first".to_string(), 100);
...
fn test_merge_self_is_idempotent() {
    use logicaffeine_data::crdt::{GCounter, MVRegister, Merge, ORSet, PNCounter};
