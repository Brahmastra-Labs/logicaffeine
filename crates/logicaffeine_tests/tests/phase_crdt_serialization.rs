...
fn test_empty_crdt_serialization() {
    use logicaffeine_data::crdt::{GCounter, MVRegister, ORSet, PNCounter, VClock};
...
fn test_lww_serialization_preserves_timestamp() {
    use logicaffeine_data::crdt::LWWRegister;

    let reg = LWWRegister::new("test value".to_string(), 1234567890);
