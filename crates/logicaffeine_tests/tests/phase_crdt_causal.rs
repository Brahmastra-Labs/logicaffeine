...
fn test_replica_id_is_u64() {
    let id = logicaffeine_data::crdt::generate_replica_id();
...
fn test_replica_id_unique() {
    let id1 = logicaffeine_data::crdt::generate_replica_id();
    let id2 = logicaffeine_data::crdt::generate_replica_id();
    assert_ne!(id1, id2);
}
...
fn test_vclock_merge_commutative() {
    use logicaffeine_data::crdt::{Merge, VClock};
...
fn test_dot_creation() {
    use logicaffeine_data::crdt::Dot;
...
fn test_dot_context_new() {
    use logicaffeine_data::crdt::DotContext;
    let ctx = DotContext::new();
    // Empty context
    assert!(!ctx.has_seen(&logicaffeine_data::crdt::Dot::new(1, 1)));
}
...
fn test_dot_context_has_seen_after_next() {
    ...
    assert!(!ctx.has_seen(&logicaffeine_data::crdt::Dot::new(42, 99)));
}
...
fn test_dot_context_merge_commutative() {
    ...
    assert!(a1.has_seen(&logicaffeine_data::crdt::Dot::new(1, 1)));
    assert!(a1.has_seen(&logicaffeine_data::crdt::Dot::new(1, 2)));
    assert!(a1.has_seen(&logicaffeine_data::crdt::Dot::new(2, 1)));
    assert!(b1.has_seen(&logicaffeine_data::crdt::Dot::new(1, 1)));
    assert!(b1.has_seen(&logicaffeine_data::crdt::Dot::new(1, 2)));
    assert!(b1.has_seen(&logicaffeine_data::crdt::Dot::new(2, 1)));
}
...
fn test_dot_context_serialization() {
    ...
    assert!(decoded.has_seen(&logicaffeine_data::crdt::Dot::new(1, 1)));
    assert!(decoded.has_seen(&logicaffeine_data::crdt::Dot::new(2, 1)));
}
...
fn test_delta_crdt_trait_exists() {
    use logicaffeine_data::crdt::DeltaCrdt;
