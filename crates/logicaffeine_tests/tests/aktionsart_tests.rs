...
use logicaffeine_language::ast::AspectOperator;
use logicaffeine_language::lexicon::{Lexicon, VerbClass};
...
fn activity_present_produces_habitual() {
    let view = common::parse_to_view("John runs.");
    match view {
...
fn activity_progressive_produces_progressive() {
    let view = common::parse_to_view("John is running.");
    match view {
        ExprView::Aspectual { operator, .. } => {
...
fn state_present_produces_simple_predication() {
    let view = common::parse_to_view("John knows Mary.");
    match view {
...
fn semelfactive_progressive_produces_iterative() {
    let view = common::parse_to_view("John is knocking.");
    match view {
        ExprView::Aspectual { operator, .. } => {
...
fn accomplishment_present_produces_habitual() {
    let view = common::parse_to_view("John builds houses.");
    match view {
        ExprView::Aspectual { operator, .. } => {
            assert_eq!(operator, AspectOperator::Habitual);
        }
...
fn achievement_present_produces_habitual() {
    let view = common::parse_to_view("John wins games.");
    match view {
        ExprView::Aspectual { operator, .. } => {
            assert_eq!(operator, AspectOperator::Habitual);
        }
...
fn state_past_produces_temporal_past() {
    let view = common::parse_to_view("John knew Mary.");
    match view {
        ExprView::NeoEvent { verb, modifiers, .. } => {
            assert_eq!(verb, "Know");
...
fn activity_past_produces_temporal_past_without_habitual() {
    let view = common::parse_to_view("John ran.");
    match view {
        ExprView::NeoEvent { verb, modifiers, .. } => {
            assert_eq!(verb, "Run");
...
fn achievement_past_produces_temporal_past() {
    let view = common::parse_to_view("John won.");
    match view {
        ExprView::NeoEvent { verb, modifiers, .. } => {
            assert_eq!(verb, "Win");
...
fn debug_verb_class_for_knowing() {
    ...
    let lex = logicaffeine_language::lexicon::Lexicon::new();
