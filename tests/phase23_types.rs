...
use logos::analysis::{TypeRegistry, TypeDef, DiscoveryPass};
use logos::{Interner, Lexer};
use logos::mwe;
...
fn type_registry_is_generic() {
    ...
    registry.register(stack, TypeDef::Generic { param_count: 1 });
    registry.register(user, TypeDef::Struct { fields: vec![], generics: vec![] });
...
fn make_tokens(source: &str, interner: &mut Interner) -> Vec<logos::Token> {
