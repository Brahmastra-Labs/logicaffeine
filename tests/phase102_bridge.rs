...
use logos::interpreter::RuntimeValue;
...
fn test_interpreter_with_kernel_context() {
    use logos::kernel::Context;
    use logos::intern::Interner;
    use logos::interpreter::Interpreter;
    use std::sync::Arc;

    // Create kernel context with standard library
    let mut ctx = Context::new();
    // Register Nat (the prelude should do this, but we do it manually for test)
    ctx.add_inductive("Nat", logos::kernel::Term::Sort(logos::kernel::Universe::Type(0)));
    ctx.add_constructor("Zero", "Nat", logos::kernel::Term::Global("Nat".to_string()));
...
fn test_interpreter_get_constructors() {
    ...
    let mut ctx = Context::new();
    ctx.add_inductive("Bool", logos::kernel::Term::Sort(logos::kernel::Universe::Type(0)));
    ctx.add_constructor("True", "Bool", logos::kernel::Term::Global("Bool".to_string()));
    ctx.add_constructor("False", "Bool", logos::kernel::Term::Global("Bool".to_string()));
