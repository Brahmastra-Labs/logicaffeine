...
fn runtime_files_are_injected() {
    let source = "## Main\nLet x: Text be \"Hello\".";
    let temp_dir = std::env::temp_dir().join("logos_phase29_test");

    if temp_dir.exists() {
        std::fs::remove_dir_all(&temp_dir).unwrap();
    }

    compile_to_dir(source, &temp_dir).expect("Compilation failed");

    // Verify logos_core was written
    assert!(temp_dir.join("logos_core/Cargo.toml").exists(), "Missing Cargo.toml");
    assert!(temp_dir.join("logos_core/src/lib.rs").exists(), "Missing lib.rs");
    assert!(temp_dir.join("logos_core/src/types.rs").exists(), "Missing types.rs");
    assert!(temp_dir.join("logos_core/src/io.rs").exists(), "Missing io.rs");

    // Verify main.rs imports prelude
    let main_rs = std::fs::read_to_string(temp_dir.join("src/main.rs")).unwrap();
...
fn runtime_has_type_aliases() {
    let temp_dir = std::env::temp_dir().join("logos_phase29_types");
    if temp_dir.exists() {
        std::fs::remove_dir_all(&temp_dir).unwrap();
    }

    compile_to_dir("## Main\nReturn.", &temp_dir).expect("Compilation failed");
...
fn runtime_has_io_functions() {
    let temp_dir = std::env::temp_dir().join("logos_phase29_io");
    if temp_dir.exists() {
        std::fs::remove_dir_all(&temp_dir).unwrap();
    }
...
fn prelude_exports_all() {
    let temp_dir = std::env::temp_dir().join("logos_phase29_prelude");
    if temp_dir.exists() {
        std::fs::remove_dir_all(&temp_dir).unwrap();
    }
