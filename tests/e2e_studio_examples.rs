...
use logos::{compile, compile_all_scopes, parse};
use logos::ast::{QuantifierKind, TemporalOperator};
...
fn parse_tree_some_dogs_bark() {
    let view = parse!("Some dogs bark loudly.");
    match view {
...
fn parse_tree_quick_brown_fox() {
    let view = parse!("The quick brown fox jumps.");
    match view {
        ExprView::NeoEvent { verb, roles, .. } => {
...
fn parse_tree_no_student_failed() {
    let view = parse!("No student failed.");
    match view {
...
fn parse_tree_future_tense() {
    let view = parse!("The train will arrive.");
