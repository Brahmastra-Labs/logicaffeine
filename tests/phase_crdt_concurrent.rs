...
fn test_pncounter_concurrent_inc_dec() {
    use logos_core::crdt::{Merge, PNCounter};
...
fn test_lww_concurrent_writes() {
    ...
    // Create registers with controlled timestamps
    let mut regs: Vec<LWWRegister<String>> = Vec::new();

    for i in 0..5 {
        let reg = LWWRegister::new(format!("value-{}", i));
        std::thread::sleep(std::time::Duration::from_millis(2));
        regs.push(reg);
    }

    // Last one should have highest timestamp
...
fn test_lww_merge_order_independence() {
    ...
    let r1 = LWWRegister::new("first".to_string());
    std::thread::sleep(std::time::Duration::from_millis(2));
