...
runs through an interpreter, a bytecode VM, and a JIT, or compiles ahead-of-time to Rust (see
[Execution & performance](execution-and-performance.md)).
...
`Text`, `Char`, and `Byte` (u8). Operators: arithmetic `+ - * / %` (also the words `plus`, `minus`,
...
`e2e_comparisons`, `e2e_operators`, `phase_bitwise`.)
...
[merge-sort example](../NEW_README.md#a-complete-example-merge-sort)); move/borrow verbs and
...
`file`, `time`, `random`, `env`, `crdt`, `concurrency`, and `net`, exposing functions like `now`,
`sleep`, `randomInt`, and `randomFloat`.
