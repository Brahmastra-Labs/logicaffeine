//! Phase 56: Distributed<T> - Unified Persistence + Network Synchronization
//!
//! The Mesh-Journal Bridge: Every network event is journaled, every local change is broadcast.
//!
//! This solves the "data loss on restart" bug where Synced<T> merges remote updates to RAM
//! but doesn't persist them to disk, so they're lost when the browser restarts.
//!
//! Local mutation flow:  RAM → Journal → Network
//! Remote update flow:   Network → RAM → Journal
    ...
    fn start_receive_loop(&self, topic: String) {
        ...
        tokio::spawn(async move {
            ...
            while let Some(bytes) = rx.recv().await {
                ...
                // CRITICAL: Persist to Journal (the whole point of Phase 56)
